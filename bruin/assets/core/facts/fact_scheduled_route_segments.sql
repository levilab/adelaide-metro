...
WITH ordered_events AS (
  SELECT
    *,
    ...
    LEAD(stop_name) OVER (PARTITION BY trip_id ORDER BY stop_sequence) AS next_stop_name,
    LEAD(stop_lat) OVER (PARTITION BY trip_id ORDER BY stop_sequence) AS next_stop_lat,
    LEAD(stop_lon) OVER (PARTITION BY trip_id ORDER BY stop_sequence) AS next_stop_lon,
    LEAD(lga_name) OVER (PARTITION BY trip_id ORDER BY stop_sequence) AS next_lga_name,
    LEAD(stop_sequence) OVER (PARTITION BY trip_id ORDER BY stop_sequence) AS next_stop_sequence,
    LEAD(scheduled_arrival_seconds) OVER (PARTITION BY trip_id ORDER BY stop_sequence) AS next_scheduled_arrival_seconds,
    LEAD(shape_dist_traveled) OVER (PARTITION BY trip_id ORDER BY stop_sequence) AS next_shape_dist_traveled
...
paired_events AS (
  SELECT
    ...
    next_stop_lat AS to_stop_lat,
    next_stop_lon AS to_stop_lon,
    next_lga_name AS to_lga_name,
    scheduled_departure_seconds,
    next_scheduled_arrival_seconds AS scheduled_arrival_seconds,
    departure_hour,
    departure_time_bucket,
    shape_dist_traveled AS from_shape_dist_traveled,
    next_shape_dist_traveled AS to_shape_dist_traveled
  FROM ordered_events
